...
The deterministic suite also executes `reddit_safari_back_button_fix.user.js` and covers the observed Safari/Macaque `navigate` challenge state, challenge-target pending/armed transitions across documents, misleading `navigate` returns into an armed zombie challenge, BFCache and `popstate` traversal fallbacks, `window.close()` then `history.back()` escape behavior, 1200 ms throttle, four-action cap, one-time stale-session reset on upgrade, and challenge cleanup including the current `jsc_token` parameter.
